...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    ...
    // On remet loading=true PENDANT le chargement du profil pour ne pas
    // afficher l'écran "aucune copropriété" avant que les données soient là.
    const { data: sub } = supabase.auth.onAuthStateChange(async (_event, sess) => {
      ...
      if (sess?.user?.id) {
        ...
        setProfile(null); setRoles([]); setBuilding(null)
      }
      if (active) setLoading(false)
    })
