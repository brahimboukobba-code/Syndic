  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {/* Bottom nav (mobile) — toutes les sections, compactes */}
        <nav
          className="md:hidden fixed bottom-0 inset-x-0 z-10 border-t border-black/5 dark:border-white/10 bg-white dark:bg-brand-700 grid"
          style={{ gridTemplateColumns: `repeat(${visibleNav.length}, minmax(0, 1fr))`, paddingBottom: 'env(safe-area-inset-bottom)' }}
        >
          {visibleNav.map((item) => (
            <NavLink
              key={item.to}
              ...
              className={({ isActive }) =>
                `flex flex-col items-center justify-center gap-0.5 py-1.5 px-0.5 ${
                  isActive ? 'text-brand-500 dark:text-sand-50' : 'opacity-55'
                }`
              }
            >
              <svg viewBox="0 0 24 24" className="h-[18px] w-[18px] fill-none stroke-current shrink-0" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                <path d={item.icon} />
              </svg>
              <span className="text-[9px] leading-tight font-medium text-center truncate w-full">{t(`navShort.${item.key}`)}</span>
            </NavLink>
          ))}
        </nav>
      </div>
